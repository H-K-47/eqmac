...
void draw_spawn_list()
{
    ...
    for (int i = 0; i < EVERQUEST_SPAWNS_MAX; i++)
    {
        ...
        std::string spawn_name = memory.read_string(spawn_info_address + EVERQUEST_OFFSET_SPAWN_INFO_NAME, EVERQUEST_SPAWN_INFO_NAME_SIZE);
    ...
    for (spawn_list_it = spawn_list_it; spawn_list_it != spawn_list.end(); spawn_list_it++)
    {
        ...
        unsigned char spawn_type = (*spawn_list_it).second.type;
...
void update_process(int value)
{
    ...
    if (everquest_hwnd == NULL)
    {
        everquest_hwnd = FindWindow(NULL, "EverQuest");
    }

    if (everquest_hwnd == NULL)
    {
        memory.set_process_hwnd(NULL);
        return;
    }

    DWORD process_id;
    GetWindowThreadProcessId(everquest_hwnd, &process_id);

    HANDLE process_handle = OpenProcess(PROCESS_ALL_ACCESS, false, process_id);

    char process_filename[MAX_PATH] = {0};
    GetModuleFileNameEx(process_handle, NULL, process_filename, sizeof(process_filename));

    //MessageBox(NULL, process_filename, "process_filename", MB_OK);

    if (strstr(process_filename, "eqgame.exe") != NULL)
    {
        //memory.set_process_by_id(process_id);
...
void update_zone(int value)
{
    ...
    if (zone_long_name.size() != 0 && zone_short_name.size() != 0)
    {
        window_title_buffer << zone_long_name << " " << "(" << zone_short_name << ")" << " - " << application_name;
    }
    else
    {
        window_title_buffer << application_name;
    }
...
/*
    if (everquest_is_graphics_dll_build_date_correct(memory) == false)
    {
        draw_color_magenta();

        draw_error_text("ERROR: EverQuest graphics DLL build date is not correct!");
...
*/
    ...
    if (map_draw_points == true)
    {
        ...
        foreach (map_point_t map_point, map_points)
        {
            ...
            if (point_map_distance > map_origin_x && point_map_distance > map_origin_y)
            {
                continue;
            }
    ...
    if (map_draw_spawns == true)
    {
        ...
        foreach (everquest_spawn_t map_spawn, map_spawns)
        {
            ...
            if (map_spawn._class >= EVERQUEST_CLASS_GUILDMASTER_BEGIN && map_spawn._class <= EVERQUEST_CLASS_GUILDMASTER_END)
            {
                ...
                {
                    unsigned char player_class = memory.read_any<BYTE>(player_spawn_info + EVERQUEST_OFFSET_SPAWN_INFO_CLASS);
