...
std::string everquest_path = "C:\EQMac";
...
void play_nice_read_ini()
{
    ...
    everquest_path = pt.get<std::string>("Options.EverQuestPath", everquest_path);

    sleep_time_foreground = pt.get<int>("Options.SleepTimeForeground", sleep_time_foreground);
    sleep_time_background = pt.get<int>("Options.SleepTimeBackground", sleep_time_background);
}
    ...
    for (i = 0; i < num_processes; i++)
    {
        ...
        if (EnumProcessModules(process_handle, &hmod, sizeof(hmod), &needed_ex))
        {
            ...
            if (strcmp(process_name, "eqgame.exe") == 0)
            {
                ...
                GetModuleFileNameEx(process_handle, hmod, process_filename, sizeof(process_filename));

                std::string process_path = process_filename;

                boost::algorithm::to_lower(everquest_path);
                boost::algorithm::to_lower(process_path);

                std::size_t found = process_path.find(everquest_path);
